...
lint: ## Run golangci-lint
	GOCACHE=/tmp/go-cache GOLANGCI_LINT_CACHE=/tmp/golangci-lint golangci-lint run
...
coverage: ## Run tests and print coverage summary
	...
	GOCACHE=/tmp/go-cache go tool cover -func=coverage.out
...
build: ## Build the fic binary into bin/
	...
	mkdir -p bin
	GOCACHE=/tmp/go-cache go build -o bin/fic .
